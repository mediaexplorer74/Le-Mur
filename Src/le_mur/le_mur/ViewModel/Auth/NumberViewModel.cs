    ...
    public class NumberViewModel : BaseViewModel
    {
        ...
        private async void onSendNumberCommand()
        {
            ...
            if (phoneNumber.Length == 10)
            {
                ...
                AuthStatus status = AuthStatus.Ok;

                try
                {
                    status = await TelegramApi.CheckAuth(phoneNumber);
                }
                catch (Exception ex)
                {
                    Debug.WriteLine("[ex] NumberViewModel - onSendNumberCommand ex.: " + ex.Message);
                }
                ...
                switch (status)
                {
                    ...
                    case AuthStatus.NeedCode: 
                        await Navigation.PushAsync(new CodePage(/*phoneNumber*/)); 
                        break;
                }
            }
        }
